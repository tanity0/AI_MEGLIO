...
export const HELP = {
  ...
  // トンマナ（§17）
  "header.reconvert": "保存された元画像（原本の dataURL・§40）から変換スタジオをやり直します。昔のプロジェクトは元画像が最大512pxに縮小保存されているため、一度「画像を開く」で元ファイルから読み直すと以降は無劣化で再変換できます。",
  "editor.backdrop": "透明部分とキャンバス周りの表示色です（§45）。市松（明/暗）・単色プリセット（ダーク/ライトグレー/白/黒/緑/マゼンタ）・任意色ピッカーから選べ、メインキャンバス・再生プレビュー・ギャラリーのサムネ/ミニプレビュー・タイムラインサムネに適用されます（設定は保存されます）。表示専用なので、PNG/GIF/シート/ゲーム書き出しの透明部分は従来どおり透明のままです。",
  ...
  // 修正タブ
  "patch.scope": "AIに編集させる範囲です。「選択範囲のみ」が最速・最安全。「全フレーム」は大きな素材では時間がかかります（CLIバックエンドではフレームごとに分割実行されます）。レイヤーを使っている場合、AI編集は各フレームの「アクティブレイヤー」だけを見て・書きます（§35）。",
  ...
  "motion.candidates": "モーション作成の主経路（§25/§42）です。ギャラリーは「プール」— 取り込んだ画像・AI生成・反転コピー・部位合成のすべてが1つの場所に並び、候補をクリックした順番がそのままフレームの順番になります（クリックで番号バッジを付与・再クリックで解除して後続の番号は自動繰り上げ）。フレーム数は選択数で決まり、「確定」で選択順にタイムラインへ追加されます（タグ範囲=選択数）。「候補を生成」はN×K個の候補をプールに未選択で追加します。気に入らない生成候補は「描き直し」（追記指示+選択列の前後を文脈に同梱）で単発再生成できます。1枚ずつの生成は重め（フルサイズ出力）なので、放置できるバッチとして codex バックエンドが推奨です。ChatGPT の画像生成を使う往復も推奨: 「GPT用依頼キットを書き出し」（依頼文はポーズ最優先・ドットの粒や質感は参照と違ってよい構成。画風はツール側の変換で合わせます）→ChatGPTに参照PNGと依頼文を貼る→出てきたシートを「画像から候補追加」（または gpt-exchange/in/ に保存で自動取り込み。選択が0件なら取り込み順に自動選択）→リテイクは「部位取り込み」で組み立て — 比較先（ベースフレーム/番号付きの選択候補）との差分から矩形やパーツチップで手足など欲しい部位だけ拾い、合成結果は新しい候補としてプールに入ります。取り込み方は「完全置換/前面（候補の非透明だけ上に乗せる）/背面（比較先の透明部にだけ差し込む=後ろ髪・マント向け）」から選べます。確定後の仕上げは、矩形選択+指示（修正タブ）やペンでラフ→部分仕上げが推奨ルートです。",
  "motion.openGallery": "AI生成を実行せずに候補ギャラリー（プール・§42）を開きます。手持ちの画像だけ取り込みたいとき（画像から候補追加・受信箱の自動取り込み・GPT依頼キット）はこちら。開いた時点で、現在のタグ（「全体」なら全フレーム）の既存フレームがプールに並び、フレーム順に選択済み（1..N）で初期化されます（§44）。この状態からの「確定」はそのタグ範囲の置き換えになります（数の増減にも対応・フレーム削除や基準フレームの変更を含むときは確認ダイアログ）。既にセッションがあればプール・選択状態を保持したまま再表示します。「画像から候補追加」では取り込み前にコマ一覧プレビューが出て、チェックを外したコマは取り込みません（gpt-exchange の in/ 自動取り込みは自動性優先でプレビューなしの全取り込み。不要分は候補の削除で対応）。取り込んだコマは、選択が0件なら取り込み順に自動選択（1..M）、既に選択があれば未選択でプールに追加されます（§39/§42）。",
  "motion.candidateAdjust": "画像由来の候補（「画像から候補追加」で取り込んだもの）の「調整」ボタンは、本物の変換スタジオを候補モードで開きます（§44）。入力は元画像の該当コマ領域（srcRegion・原寸）、初期つまみは前回の設定（convParams）。解像度（高さ）と色数はプロジェクトに固定され、それ以外——背景除去閾値・フチ光彩除去・セルサイズ±・グリッド位相/再推定・支配色/セル中心・輪郭/彩度保護・候補3×3・同期ズーム——は通常の変換スタジオと同じです。「確定（候補へ反映）」でプロジェクトパレットへスナップ→足元/重心整列し直して候補に入ります（ナッジ済みオフセットは維持・キャンセルで無変更）。「自動調整」ボタン（候補モードのみ表示）は、変換結果を元絵と比較するスコア（色近似+輪郭一致+欠け/残りペナルティ）でつまみを自動探索して最良値をセットします（1コマ最大40変換・以降手で微調整可・§43）。取り込みプレビューの「自動調整」トグル（既定ON）を入れておくと、取り込み確定時に全コマへ同じ探索が自動で走ります（進捗表示あり・「自動調整を中断」で残りをスキップして取り込み続行）。グリッド生成（AI生成）や既存フレーム由来の候補には調整ボタンは出ません（既存フレームの再変換はヘッダの「再変換」で）。",
  ...
  "rig.partEdit": "パーツタブ表示中は、キャンバス上のパーツ枠（±4px）をクリックしてパーツを選択できます。選択中は枠内ドラッグで矩形の移動、四隅・四辺の8ハンドルでリサイズ、pivot の十字ドラッグで支点移動ができます（1セル単位・キャンバス内にクランプ・Ctrl+Zで戻せます）。Esc または何もない場所のクリックで選択解除。矩形を変えるとベースフレームから切り出し直され、次のフレーム生成から反映されます。",
