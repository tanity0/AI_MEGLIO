...
export const HELP = {
  ...
  "motion.candidates": "モーション作成の主経路（§25）です。フレームごとにK個の候補を並列生成し、ギャラリーで見比べて採用→全フレーム採用で確定するとタグ付きでタイムラインに追加されます。気に入らない候補は「描き直し」（追記指示+採用済み前後フレームを文脈に同梱）や「追加生成」で増やせます。1枚ずつの生成は重め（フルサイズ出力）なので、放置できるバッチとして codex バックエンドが推奨です。ChatGPT の画像生成を使う往復も推奨: 「GPT用依頼キットを書き出し」（依頼文はポーズ最優先・ドットの粒や質感は参照と違ってよい構成。画風はツール側の変換で合わせます）→ChatGPTに参照PNGと依頼文を貼る→出てきたシートを「画像から候補追加」（または gpt-exchange/in/ に保存で自動取り込み）→リテイクは「部位取り込み」で組み立て — テイスト・身長感・ポーズの全部が揃う候補は無い前提で、顔や胴はベース（比較先）を守り、候補からは矩形やパーツチップで手足など欲しい部位だけ拾います。取り込み方は「完全置換/前面（候補の非透明だけ上に乗せる）/背面（比較先の透明部にだけ差し込む=後ろ髪・マント向け）」から選べます。確定後の仕上げは、矩形選択+指示（修正タブ）やペンでラフ→部分仕上げが推奨ルートです。",
  "motion.openGallery": "AI生成を実行せずに候補ギャラリーを開きます。手持ちの画像だけ取り込みたいとき（画像から候補追加・受信箱の自動取り込み・GPT依頼キット）はこちら。既にセッションがあれば生成済み候補・採用状態を保持したまま再表示します。「画像から候補追加」では取り込み前にコマ一覧プレビューが出て、チェックを外したコマは取り込みません（gpt-exchange の in/ 自動取り込みは自動性優先でプレビューなしの全取り込み。不要分は候補の削除で対応）（§39）。",
