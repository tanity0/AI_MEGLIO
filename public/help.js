...
export const HELP = {
  ...
  "motion.candidates": "モーション作成の主経路（§25）です。フレームごとにK個の候補を並列生成し、ギャラリーで見比べて採用→全フレーム採用で確定するとタグ付きでタイムラインに追加されます。気に入らない候補は「描き直し」（追記指示+採用済み前後フレームを文脈に同梱）や「追加生成」で増やせます。1枚ずつの生成は重め（フルサイズ出力）なので、放置できるバッチとして codex バックエンドが推奨です。ChatGPT の画像生成を使う往復も推奨: 「GPT用依頼キットを書き出し」（依頼文はポーズ最優先・ドットの粒や質感は参照と違ってよい構成。画風はツール側の変換で合わせます）→ChatGPTに参照PNGと依頼文を貼る→出てきたシートを「画像から候補追加」（または gpt-exchange/in/ に保存で自動取り込み）→リテイクは「部位取り込み」で組み立て — テイスト・身長感・ポーズの全部が揃う候補は無い前提で、顔や胴はベース（比較先）を守り、候補からは矩形やパーツチップで手足など欲しい部位だけ拾います。確定後の仕上げは、矩形選択+指示（修正タブ）やペンでラフ→部分仕上げが推奨ルートです。",
