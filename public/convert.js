...
const FLATTEN_LEVELS = {
  // §101: 多数決のみ。need = 9マス中いくつ同じ色があれば置き換えるか。
  // 色を動かす処理（RGB中央値→最近色）は §99 で試したが、平らさが同じでも
  // コントラストが30%落ちて絵が滲むため廃止した（§101.1）。
  1: { need: 5, passes: 1 }, // 弱:   孤立ドットだけ消す
  2: { need: 4, passes: 2 }, // 強:   面をまとめる
  3: { need: 3, passes: 2 }, // 最強: さらにまとめる（コントラストが1割強落ちる）
};
...
export function flattenIndexed(pixels, w, h, level) {
...
export function applyFlatten(res, level) {
  ...
  if (Array.isArray(res.framesPixels)) {
    res.framesPixels = res.framesPixels.map((px) => flattenIndexed(px, w, h, level));
    res.framesPixels.forEach(tally);
    res.pixels = res.framesPixels[0];
  } else if (res.pixels) {
    res.pixels = flattenIndexed(res.pixels, w, h, level);
    tally(res.pixels);
  }
  res.counts = counts;
  return res;
