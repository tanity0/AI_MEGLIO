...
export const HELP = {
  ...
  "motion.openGallery": "AI生成を実行せずに候補ギャラリー（プール・§42）を開きます。手持ちの画像だけ取り込みたいとき（画像から候補追加・受信箱の自動取り込み・GPT依頼キット）はこちら。既にセッションがあればプール・選択状態を保持したまま再表示します。「画像から候補追加」では取り込み前にコマ一覧プレビューが出て、チェックを外したコマは取り込みません（gpt-exchange の in/ 自動取り込みは自動性優先でプレビューなしの全取り込み。不要分は候補の削除で対応）。取り込んだコマは、選択が0件なら取り込み順に自動選択（1..M）、既に選択があれば未選択でプールに追加されます（§39/§42）。",
  "motion.candidateAdjust": "画像由来の候補（「画像から候補追加」で取り込んだもの）を、取り込み後にもう一度つまみで調整できます。「調整」ボタンで開くパネルは変換スタジオのフレーム別つまみ（§30）と同じ項目——背景除去閾値・フチ光彩除去・輪郭保護・彩度保護・セルサイズ微調整——を持ち、変更のたび元画像の該当コマ領域（srcRegion・原寸）から再変換 → プロジェクトパレットへスナップ → 足元/重心整列をやり直してライブプレビューします。ナッジ済みのオフセットは維持されるので、位置合わせをやり直す必要はありません。「適用」で確定、「キャンセル」で破棄。パレットは常にプロジェクトパレット固定（色数・中身とも不変）。グリッド生成（AI生成）の候補にはこのボタンは出ません。取り込み前のプレビュー段階でも、全コマ共通の簡易版（背景除去閾値・フチ光彩）で変換し直せます（§41）。さらに「自動」ボタンで、このコマのつまみを『元絵に一番近い見栄え』になる値へ自動探索できます（変換結果を元絵と比較するスコア＝色近似+輪郭一致+欠け/残りペナルティで評価・1コマ最大40変換・選ばれた値はつまみに入るので手で微調整可）。取り込みプレビューの「自動調整」トグル（既定ON）を入れておくと、取り込み確定時に全コマへ同じ探索が自動で走ります（進捗表示あり・「自動調整を中断」で残りをスキップして取り込み続行・§43）。",
