...
export const HELP = {
  ...
  "motion.candidates": "モーション作成の主経路（§25）です。フレームごとにK個の候補を並列生成し、ギャラリーで見比べて採用→全フレーム採用で確定するとタグ付きでタイムラインに追加されます。気に入らない候補は「描き直し」（追記指示+採用済み前後フレームを文脈に同梱）や「追加生成」で増やせます。1枚ずつの生成は重め（フルサイズ出力）なので、放置できるバッチとして codex バックエンドが推奨です。確定後の仕上げは、矩形選択+指示（修正タブ）やペンでラフ→部分仕上げが推奨ルートです。",
