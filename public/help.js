...
export const HELP = {
  ...
  "rig.redraw": "リグ合成の結果を「ポーズの設計図」として、線・シェーディング・ディテールをベースフレームに合わせてAIが描き直します。描き込みの多いキャラで機械回転が破綻するときの本命です。仕上げにAI清書を掛けるのが推奨ルートです。所要時間は対象領域のサイズにほぼ比例するので、時間がかかるときはパーツの「固定」で回転パーツを減らすか「回転した部位のみ」で領域を絞ってください。",
